        ...
        <div className="max-w-5xl mx-auto flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="font-bold text-slate-900">Compréhension de l&apos;Écrit</h1>
            </div>
            <p className="text-xs text-slate-400 mt-0.5">{tests.length} Séries · 39 questions/série · 60 min/série</p>
          </div>
          <div className="ml-auto flex items-center gap-1 bg-slate-100 rounded-xl p-1">
            <button className="px-4 py-1.5 rounded-lg bg-white shadow-sm text-sm font-medium text-slate-800">Toutes</button>
            <button className="px-4 py-1.5 rounded-lg text-sm text-slate-500 hover:text-slate-700">Terminées</button>
            <button className="px-4 py-1.5 rounded-lg text-sm text-slate-500 hover:text-slate-700">Non terminées</button>
          </div>
        </div>
      </header>
            ...
            return (
              <Link key={test.testNumber} href={`/exam/reading/${test.testNumber}`}
                className={`group relative bg-white rounded-2xl border-2 p-5 hover:shadow-lg transition-all duration-200 ${
                  isDone ? "border-emerald-200 hover:border-emerald-300" : "border-slate-200 hover:border-emerald-300"
                }`}>
                <div className="flex items-start justify-between mb-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm ${
                    isDone ? "bg-emerald-100 text-emerald-600" : "bg-emerald-100 text-emerald-600 group-hover:bg-emerald-200 transition-colors"
                  }`}>
                ...
                <p className="text-xs text-slate-400">39 questions · 60 min</p>
                <div className={`mt-4 flex items-center gap-2 text-sm font-semibold ${isDone ? "text-emerald-600" : "text-emerald-600"}`}>
