...
export default function TeacherListeningBankPage() {
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
            <Link href="/teacher/exams" className="hover:text-blue-600">Ngân hàng đề</Link>
            <span>/</span>
          </div>
        ...
        <div className="flex items-center gap-3 flex-wrap justify-end">
          ...
          <select
            ...
            className="border border-gray-300 rounded-lg px-3 py-2 text-sm font-semibold bg-white focus:ring-2 focus:ring-sky-300 focus:border-sky-500"
          >
          ...
          <Link
            href={`/teacher/assignments/new?type=listening&id=${selectedSerie}&label=${encodeURIComponent(`Série ${selectedSerie}`)}`}
      ...
      {questions.length > 0 && (
        <div className="flex gap-3 mb-5 flex-wrap">
          <span className="text-xs font-bold bg-sky-100 text-sky-700 px-3 py-1.5 rounded-full">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          {/* Table header */}
          <div className="grid grid-cols-[50px_60px_60px_1fr_50px_50px_60px] gap-2 px-4 py-3 bg-gray-50 border-b border-gray-200 text-xs font-bold text-gray-500 uppercase tracking-wider">
            <span>#</span>
            <span>Level</span>
            <span>Pts</span>
            <span>Question</span>
              ...
              return (
                <div key={q.id}>
                  ...
                  <button
                    ...
                    className="w-full grid grid-cols-[50px_60px_60px_1fr_50px_50px_60px] gap-2 px-4 py-3 hover:bg-sky-50/50 transition-colors text-left items-center"
                  >
                    ...
                    <span>{q.audioUrl ? <Volume2 className="w-4 h-4 text-sky-500" /> : <span className="text-gray-300">—</span>}</span>
                  ...
                  {/* Expanded detail */}
                  {isExpanded && (
                    <div className="px-6 pb-5 pt-1 bg-sky-50/30 border-t border-sky-100">
                      {/* Full prompt */}
                      <div className="mb-3">
                      ...
                      {q.audioUrl && (
                        <div className="mb-3">
                          <p className="text-xs font-bold text-gray-400 uppercase mb-1">Audio</p>
                          <audio controls className="h-9" src={q.audioUrl} />
                        </div>
                      )}
                      ...
                      {q.imageUrl && (
                        <div className="mb-3">
                          ...
                          <div className="flex justify-center">
                            ...
                            <img src={q.imageUrl} alt="Question" className="max-w-xs rounded-lg" />
                          </div>
                        </div>
                      )}
                      ...
                      {transcriptions[q.id] && (
                        <div className="mt-4 pt-4 border-t border-gray-100">
                          <p className="text-xs font-bold text-gray-400 uppercase mb-2 flex items-center gap-1">
                            <FileText className="w-3.5 h-3.5" /> Transcription (Lời thoại)
                          </p>
                          <div className="bg-white border border-sky-100 rounded-lg p-4 shadow-sm">
                            <p className="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed italic border-l-2 border-sky-300 pl-4">
                              {transcriptions[q.id]}
                            </p>
                          </div>
                        </div>
